...
* And finally showcasing how the typical way WAL encryption is implemented in TDE solutions can cause performance degradation in high WAL-churn scenarios
...
The following table shows some single threaded measurements on my test computer:

| What | Bandwidth (GB/s) | Description |
| --- | --- | --- |
| Memory read | 23 | How quickly can we read the memory |
| Memory copy  | 11 | How quickly can we copy data in the memory from one place to another |
...
The 3 mentioned encryption algorithms are commonly used in many scenarios:
...
We also have to remember that our goal isn't encrypting a random stream in memory, but somehow we have to integrate it into an already existing database system with its preexisting architecture.
...
With AES-NI, encryption operations often don't require additional memory bandwidth, which means that if a workload is already disk or memory limited, but we have free CPU cycles, we might get encryption for free or little cost.
...
A typical data-at-rest encryption implementation usually operates at the IO level: it encrypts immediately before we write a dirty buffer to disk, and it reads immediately after we read something from disk.

Reads only happen when we actually have to read from disk. If the requested pages are already in the shared buffers, they are already decrypted, no effect there.
Writes only happen when we are writing to disk, which means that in a not so write-heavy workload where all writes happen in the checkpointer and background writer and backend processes do not actually perform page writes directly, it won't affect the QPS numbers at all.
...
We might decide to keep the small dataset, and just use a small number for shared buffers, but then we are moving away from a production-like setup: is it realistic to run postgres with 128MB shared buffers in a real environment?
...
In my tests, I see the following performance degradation with them:
...
PostgreSQL has an option called [full page writes](https://wiki.postgresql.org/wiki/Full_page_writes), which is enabled by default.
...
With 15000 QPS, that's around 1.2 GB/s of WAL.
From the table at the beginning, on my test PC AES-128-CTR has around 8 GB/s single core bandwidth.
1.2GB/s is 15% of that - and that assumes no context switching or other losses, meaning no matter how good the integration into the database is, if we do encryption at this volume while holding a global lock, we have to expect at least 15% performance drop with these numbers.
...
If you have questions or suggestions about the topic, don't hesitate to reach out using our forums!
